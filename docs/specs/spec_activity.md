...
> **Статус модуля в v3:** Legacy. `ActivityExecutor` используется только в `src/scheduler/scheduler.py` (не активен). В актуальном deployment (runtime v3) лайки, комменты, подписки и репосты исполняются через `PlanExecutor` → `SDK` → `browser_actions/browser_engage`. Выбор целей — `DeterministicPlanGenerator` + `PersonaPolicy`. Задержки — `src/runtime/behavior.py`. Текст постов и комментов пишет агент (Claude Code сессия).

Модуль Activity предоставляет legacy-инструменты для взаимодействия с постами: выбор целей (`TargetSelector`), рандомизация (`HumanRandomizer`), AI-генерация комментариев (`CommentGenerator`). Эти компоненты могут быть полезны вне runtime, но в текущем control flow не задействованы.
...
> **Актуальное состояние (v3):** `BapiClient.comment_post()`, `BapiClient.repost()` и `BapiClient.create_post()` — stubs (NotImplementedError). Bapi-эндпоинты для этих действий не обнаружены. Реальное исполнение комментариев, репостов и постов идёт через `browser_actions` (Playwright CDP). `browse_and_interact()` удалён — агент сам решает что делать.

Рабочий путь: `sdk.comment_on_post()` → `browser_actions.comment_on_post()` (DOM).
Рабочий путь: `sdk.create_post()` → `browser_actions.create_post()` (DOM).
Рабочий путь: `sdk.quote_repost()` → `browser_actions.repost()` (DOM).

ActivityExecutor из этого модуля — legacy оркестратор. В v3 runtime используется `PlanExecutor` из `src/runtime/plan_executor.py`.
