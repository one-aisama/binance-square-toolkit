...
The Accounts module provides tools for loading account/persona configurations from YAML, controlling daily action limits, and preventing cross-interactions between accounts. The agent uses this module to know which accounts exist, what their personas and limits are, and to check whether an action is allowed before execution.
...
- As an agent, I want every action (success or failure) to be recorded in the DB, so activity can be tracked and debugged.
- As an agent, I want to verify that the target post was not authored by one of my own accounts, so I never create self-interactions.
...
**Deterministic daily limit:** for each triple `(account_id, date, action_type)` a seed hash determines the exact limit within the range `[min, max]`. A given account gets the same limit throughout the day but different limits on different days. Prevents predictable patterns.
...
Files starting with `_` (e.g. `_example.yaml`) are skipped. If a persona_id from an account config doesn't exist in personas.yaml, a warning is logged but the account is still loaded (with `persona = None`).
...
Agent wants to like a post for account X
  ...
  -> after a successful like: limiter.record_action("X", "like", target_id="post123")
```

### Daily Stats Upsert
`record_action` writes to both tables: `actions_log` (individual row) and `daily_stats` (upsert increment). The updated column depends on `action_type` (mapped to `{type}s_count`), and a `status` of failed increments `errors_count`.
...
|----------|-------------------|
| No account YAML files in the directory | `load_accounts` returns an empty list, logs a warning |
| Account references a non-existent persona_id | Warning logged, account loaded with `persona = None` |
...
| `record_action` without an existing daily_stats row | INSERT creates a new row via UPSERT |
...
- **Blocks:** `src/activity/` (ActivityExecutor uses ActionLimiter), `src/scheduler/` (loads accounts on startup)
