...
async def test_reserve_topic_conflict(db_path):
    ...
    first = await reserve_topic(
        db_path, agent_id="aisama", reservation_key=key, post_family="market_chart",
    )
    second = await reserve_topic(
        db_path, agent_id="sweetdi", reservation_key=key, post_family="market_chart",
    )
...
async def test_release_then_reserve(db_path):
    ...
    await reserve_topic(db_path, agent_id="aisama", reservation_key=key, post_family="news_reaction")
    await release_reservation(db_path, agent_id="aisama", reservation_key=key)
    result = await reserve_topic(db_path, agent_id="sweetdi", reservation_key=key, post_family="news_reaction")
    assert result is True
...
async def test_confirm_deletes_reservation(db_path):
    ...
    await reserve_topic(db_path, agent_id="aisama", reservation_key=key, post_family="market_chart")
    await confirm_reservation(db_path, agent_id="aisama", reservation_key=key)

    # Key is now free — another agent (or same agent later) can reserve it
    result = await reserve_topic(db_path, agent_id="sweetdi", reservation_key=key, post_family="market_chart")
    assert result is True
...
async def test_repeated_confirm_then_reserve(db_path):
    ...
    result = await reserve_topic(db_path, agent_id="aisama", reservation_key=key, post_family="market_chart")
    assert result is True
    await confirm_reservation(db_path, agent_id="aisama", reservation_key=key)
...
async def test_cleanup_expired(db_path):
    ...
    async with aiosqlite.connect(db_path) as db:
        await db.execute(
            """INSERT INTO topic_reservations
               ...
               VALUES (?, ?, ?, ?, ?)""",
            ("aisama", "OLD:macro:expired1", "market_chart", expired.isoformat(), expired.isoformat()),
        )
        await db.commit()
    ...
    active = await get_active_reservations(db_path, exclude_agent_id="sweetdi")
    assert len(active) == 0


@pytest.mark.asyncio
async def test_get_active_reservations_excludes_own(db_path):
    await reserve_topic(db_path, agent_id="aisama", reservation_key="SOL:rotation:own1", post_family="news_reaction")
    await reserve_topic(db_path, agent_id="sweetdi", reservation_key="BTC:macro:own2", post_family="market_chart")

    aisama_sees = await get_active_reservations(db_path, exclude_agent_id="aisama")
    sweetdi_sees = await get_active_reservations(db_path, exclude_agent_id="sweetdi")

    assert len(aisama_sees) == 1
    assert aisama_sees[0]["agent_id"] == "sweetdi"
    assert len(sweetdi_sees) == 1
    assert sweetdi_sees[0]["agent_id"] == "aisama"


@pytest.mark.asyncio
async def test_release_all_agent_reservations(db_path):
    await reserve_topic(db_path, agent_id="aisama", reservation_key="A:macro:r1", post_family="market_chart")
    await reserve_topic(db_path, agent_id="aisama", reservation_key="B:ta:r2", post_family="market_chart")
    await reserve_topic(db_path, agent_id="sweetdi", reservation_key="C:rotation:r3", post_family="news_reaction")

    await release_all_agent_reservations(db_path, agent_id="aisama")

    active = await get_active_reservations(db_path, exclude_agent_id="nobody")
    assert len(active) == 1
    assert active[0]["agent_id"] == "sweetdi"
