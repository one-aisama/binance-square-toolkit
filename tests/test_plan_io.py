...
def test_save_and_load_pending_plan(plan_dir):
    ...
    assert "aisama" in path
    loaded = load_pending_plan("aisama")
    assert loaded["agent_id"] == "aisama"
...
def test_update_pending_plan_adds_text(plan_dir):
    plan = _make_plan()
    save_pending_plan(agent_id="aisama", plan=plan, directive=FakeDirective(), context_files={})
    ...
    update_pending_plan("aisama", actions)
    reloaded = load_pending_plan("aisama")
...
def test_load_plan_for_execution_with_text(plan_dir):
    ...
    update_pending_plan("aisama", loaded["actions"])

    agent_plan = load_plan_for_execution("aisama")
...
def test_load_plan_for_execution_without_text_raises(plan_dir):
    ...
    with pytest.raises(ValueError, match="without text"):
        load_plan_for_execution("aisama")


def test_delete_pending_plan(plan_dir):
    plan = _make_plan()
    save_pending_plan(agent_id="aisama", plan=plan, directive=FakeDirective(), context_files={})
    delete_pending_plan("aisama")

    with pytest.raises(FileNotFoundError):
        load_pending_plan("aisama")
