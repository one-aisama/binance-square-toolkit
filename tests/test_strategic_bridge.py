...
def test_load_directive_valid(tmp_path, monkeypatch):
    runtime_dir = tmp_path / "data" / "runtime" / "aisama"
...
def test_find_latest_context_summary(tmp_path, monkeypatch):
    ...
    (context_dir / "20260404T120000Z_aisama.md").write_text("older", encoding="utf-8")
    (context_dir / "20260404T130000Z_aisama.md").write_text("newer", encoding="utf-8")
    (context_dir / "20260404T130000Z_sweetdi.md").write_text("other agent", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
    result = _find_latest_context_summary("aisama")
    assert result is not None
    assert "130000Z_aisama" in result
