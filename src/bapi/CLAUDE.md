# Module: bapi
# Purpose: HTTP client for Binance bapi — credential injection, rate limiting, retry, convenience methods
...
## Dependencies
- Uses: `session.credential_store` (CredentialStore — loads cookies+headers)
...
## Common Tasks
- Add an endpoint: constant in `endpoints.py`, method in `BapiClient`
- Debug auth errors: check `credential_store.load()`, look for 401/403 in logs

## Known Issues
- `create_post`, `comment_post`, `repost` — stubs (endpoints not found, browser is used instead)
