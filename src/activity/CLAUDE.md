...
|------|-------|--------------|
| executor.py | 132 | ActivityExecutor — like/comment/repost loop, limit checks, stub handling |
...
## Dependencies
- Uses: `bapi.client` (like_post works; comment_post/repost — stubs)
- Uses: `accounts.limiter` (ActionLimiter — check and record every action)
...
## Known Issues
- comment_post and repost in BapiClient are stubs; real actions go through `session.browser_actions`
- comment_gen loads `config/content_rules.yaml`, but rules are not yet used in prompts
- The software does not generate content on its own — CommentGenerator is a tool that the agent invokes
