...
|------|-------|--------------|
| manager.py | 94 | load_accounts() + load_personas() — reads YAML, merges persona into AccountConfig via Pydantic |
| limiter.py | 87 | ActionLimiter — checks daily counters vs randomized limit, writes to actions_log + daily_stats |
| anti_detect.py | 16 | are_own_accounts() + should_skip_post_by_author() — prevents cross-account interactions |
...
- Change limits: `limits` block in the account YAML or `LimitsConfig` defaults
