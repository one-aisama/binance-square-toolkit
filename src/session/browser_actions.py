...
SPAM_KEYWORDS = {"gift", "giveaway", "airdrop", "copy trading"}
...
async def browse_and_interact(
    ws_endpoint: str,
    comment_generator=None,  # CommentGenerator instance from activity.comment_gen
    count: int = 5,
    skip_rate: float = 0.3,
) -> dict[str, Any]:
    """DEPRECATED. Use SDK methods directly: get_feed_posts + like_post + comment_on_post + follow_user.

    Agent should make its own decisions about which posts to interact with.
    This function hardcodes decisions that belong to the agent.

    Flow:
    1. Navigate to Square, click "Recommended" tab
    2. Scroll down to load posts (6 scrolls, 800px each, 2s between)
    3. Collect post IDs from feed
    4. Visit each post, check quality:
       - Skip if text < 50 chars
       - Skip if likes < 3
       - Skip if contains: gift, giveaway, airdrop, copy trading
    5. For good posts: like + AI-generated comment + follow
    6. Human delay between posts (15-35s, increasing)
    ...
    try:
        logger.info("Navigating to Binance Square feed...")
        await page.goto(page_map.SQUARE_URL, wait_until="domcontentloaded", timeout=60_000)
        await asyncio.sleep(5)

        # Click Recommended tab
        rec_tab = page.locator(page_map.FEED_RECOMMENDED_TAB).first
        try:
            await rec_tab.click(timeout=5_000)
            await asyncio.sleep(3)
        except Exception:
            logger.warning("Recommended tab not found, using default feed")

        # Scroll to load posts (6 scrolls, 800px each)
        for _ in range(6):
            await page.mouse.wheel(0, 800)
            await asyncio.sleep(2)

        # Collect post links from feed
        post_links = await page.query_selector_all("a[href*='/square/post/']")
        post_ids: list[str] = []
        seen = set()
        for link in post_links:
            href = await link.get_attribute("href") or ""
            # Extract post ID from URL like /en/square/post/12345
            parts = href.rstrip("/").split("/")
            if parts and parts[-1].isdigit():
                pid = parts[-1]
                if pid not in seen:
        ...
        # Visit and interact with posts
        for i, pid in enumerate(post_ids):
            if interacted >= count:
                break

            # Random skip for more natural behavior
            if random.random() < skip_rate:
                skipped += 1
                continue

            post_url = page_map.POST_URL_TEMPLATE.format(post_id=pid)
            logger.info(f"Visiting post {pid} ({i+1}/{len(post_ids)})...")
            await page.goto(post_url, wait_until="domcontentloaded", timeout=60_000)
            await asyncio.sleep(4)

            # Get post text for quality check and AI comment generation
            post_text = await page.evaluate(r'''() => {
                let text = '';
            ...
            if not post_text:
                post_text = await page.text_content("article") or await page.text_content("body") or ""

            # Get author name for AI comment context
            ...
            if len(post_text.strip()) < 50:
                logger.info(f"Skipping post {pid}: text too short")
                skipped += 1
                continue

            text_lower = post_text.lower()
            if any(kw in text_lower for kw in SPAM_KEYWORDS):
                logger.info(f"Skipping post {pid}: spam keyword detected")
                skipped += 1
                continue

            # Check likes count (try to find like count element)
            try:
                like_el = page.locator(page_map.POST_LIKE_BUTTON).first
                like_text = await like_el.text_content() or "0"
                like_count = int("".join(c for c in like_text if c.isdigit()) or "0")
                if like_count < 3:
                    logger.info(f"Skipping post {pid}: only {like_count} likes")
                    skipped += 1
                    continue
            except Exception:
                pass  # Can't read likes — proceed anyway
            ...
            try:
                ...
                logger.warning(f"Like failed on {pid}: {e}")

            # --- Interact: Comment (AI-generated) ---
            comment_text = ""
            if comment_generator and post_text.strip():
                try:
                    comment_text = await comment_generator.generate(post_text, author_name)
                except Exception as e:
                    logger.warning(f"AI comment generation failed for {pid}: {e}")
            if not comment_text:
                logger.info(f"Skipping comment on {pid}: no text generated")
            ...
            if comment_text:
                try:
                    ...
                    await reply_input.click()
                    await asyncio.sleep(0.5)
                    await reply_input.type(comment_text, delay=60)
                    await asyncio.sleep(2)
                    ...
                    # Handle Follow & Reply popup
                    follow_reply_btn = page.locator(page_map.POST_FOLLOW_REPLY_POPUP).first
                    try:
                        await follow_reply_btn.wait_for(state="visible", timeout=3_000)
                        await follow_reply_btn.click()
                        await asyncio.sleep(3)
                        logger.info(f"Comment sent (Follow & Reply) on {pid}")
                    except Exception:
                        logger.info(f"Comment sent on {pid}")
                except Exception as e:
                    logger.warning(f"Comment failed on {pid}: {e}")

            # --- Interact: Follow ---
            # Check ALL buttons — only click if text is exactly "Follow", not "Following"
            try:
                follow_btns = await page.query_selector_all('button')
                for btn in follow_btns:
                    btn_text = (await btn.text_content() or '').strip()
                    if btn_text == 'Follow':
                        await btn.click()
                        await asyncio.sleep(2)
                        logger.info(f"Followed author of {pid}")
                        break
            except Exception:
                pass  # No follow button or already following

            interacted += 1
            actions_log.append({"post_id": pid, "actions": ["like", "comment", "follow"]})

            # Human delay between posts (15-35s, increasing with index)
            if interacted < count:
                delay = random.uniform(15, 35) + (interacted * 2)
                logger.info(f"Waiting {delay:.0f}s before next post...")
                await asyncio.sleep(delay)

        logger.info(f"Browse complete: {interacted} interacted, {skipped} skipped")
        return {
            "success": True,
        ...
        logger.error(f"Browse and interact failed: {e}")
        return {"success": False, "error": str(e), "interacted": interacted, "actions": actions_log}
    finally:
        await pw.stop()
