...
- Test names describe WHAT is being checked, not implementation details.
- One assert per test where practical.
- Use fixtures for shared setup (mock BapiClient, mock CredentialStore, test DB).
- Integration tests with live services (e.g., `test_harvester_integration.py`) go in separate files, excluded from default run.
- Run tests: `python -m pytest tests/ -v --ignore=tests/test_harvester_integration.py`
- Current test count: 390.
